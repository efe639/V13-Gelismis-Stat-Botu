...
module.exports = {
    ...
    run: async (client, message, args, arvisbosEmbed) => {
        ...
        arvisbosEmbed.setDescription(`${kullanıcı.user.toString()} kullanıcısının genel sunucu ses ve mesaj istatistikleri;`);
        arvisbosEmbed.addField("❯ Kategori Bilgileri:", `
\`•\` Toplam: \`${voiceData ? client.getTime(voiceData.topStat) : 0}\`
${parents.length > 0 ? parents.map((e) => `\`•\` ${e.name}: \`${client.getTime(e.data)}\` `).join("\n") : ""}
`);
        arvisbosEmbed.addField(`❯ Kanal Sıralaması: (Toplam ${kanalSayısı} Kanal)`, voiceTop);
        arvisbosEmbed.addField("❯ Mesaj Bilgileri:", `\`•\` Toplam: \`${messageData ? messageData.topStat : "`Veri bulunamadı!`"}\` \n\`•\` 1 Gün: \`${messageData ? messageData.dailyStat : "`Veri bulunamadı!`"}\` \n\`•\` 1 Hafta: \`${messageData ? messageData.weeklyStat : "`Veri bulunamadı!`"}\` \n\`•\` 2 Hafta: \`${messageData ? messageData.twoWeeklyStat : "`Veri bulunamadı!`"}\``, true);
        arvisbosEmbed.addField("❯ Ses Bilgileri:", `\`•\` Toplam: \`${voiceData ? client.getTime(voiceData.topStat) : "`Veri bulunamadı!`"}\`\n\`•\` 1 Gün: \`${voiceData ? client.getTime(voiceData.dailyStat) : "`Veri bulunamadı!`"}\` \n\`•\` 1 Hafta: \`${voiceData ? client.getTime(voiceData.weeklyStat) : "`Veri bulunamadı!`"}\` \n\`•\` 2 Hafta: \`${voiceData ? client.getTime(voiceData.twoWeeklyStat) : "`Veri bulunamadı!`"}\``, true);
        arvisbosEmbed.addField(`❯ Mesaj Sıralaması: (Toplam: ${messageData ? messageData.topStat : 0})`, messageTop);
