...
module.exports = {
    conf: {
        ...
        name: "puan",
        help: "puan [ekle-çıkar-gönder] [@arvisbos/ID] [Miktar]",
        category: "Yetkili",
        enabled: true
    },
    ...
    run: async (client, message, args, arvisbosEmbed) => {
        ...
        if (args[0] === "ekle" || args[0] === "add") {
            ...
            if (pointData && client.ranks.some((e) => pointData.points >= e.points && !kullanıcı.hasRole(e.role))) {
                ...
                message.guild.channels.cache.get(config.rankLog).send(arvisbosEmbed.setDescription(`${kullanıcı.toString()} kullanıcısına ${message.member.toString()} tarafından \`${puan}\` puan eklendi ve ${roles.filter(e => roles.indexOf(e) === roles.length-1).map(e => `<@&${e.role}>`).join("\n")} rolleri verildi!`));
            }
            message.channel.send(arvisbosEmbed.setDescription(`${kullanıcı.toString()} kullanıcısına \`${puan}\` adet puan eklendi! \n\n${addedRoles.length > 0 ? `Verilen roller: \n${addedRoles.filter(e => addedRoles.indexOf(e) === addedRoles.length-1).map(e => `<@&${e.role}>`).join("\n")}` : ""}`)).then((e) => e.delete({ timeout: 10000 }) && message.react(config.emojis.onay));
			...
			if (pointData && client.ranks.some(e => pointData.points < e.points && kullanıcı.hasRole(e.role))) {
				...
				message.guild.channels.cache.get(config.rankLog).send(arvisbosEmbed.setDescription(`${kullanıcı.toString()} kullanıcısından ${message.member.toString()} tarafından \`${puan}\` adet puan çıkarıldı ve kişiden ${roles.map(e => `<@&${e.role}>`).join(", ")} rolleri alındı!`));
			}
			message.channel.send(arvisbosEmbed.setDescription(`${kullanıcı.toString()} kullanıcısından \`${puan}\` adet puan çıkarıldı! \n\n${removedRoles.length > 0 ? `Alınan roller: \n${removedRoles.map(e => `<@&${e.role}>`).join("\n")}` : ""}`)).then((e) => e.delete({ timeout: 10000 }) && message.react(config.emojis.onay));
			...
			message.channel.send(arvisbosEmbed.setDescription(`${kullanıcı.toString()} kişisine başarıyla \`${puan}\` puan gönderildi!`)).then((e) => e.delete({ timeout: 10000 }) && message.react(config.emojis.onay));
        }
    }
};
